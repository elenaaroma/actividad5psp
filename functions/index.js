...
exports.getAllElements = onRequest(async (request, response) => {
  try {
    ...
    logger.error('Error al obtener los elementos.', { error: error });
  ...
  exports.archiveDeletedElementWithCustomDate = functions.firestore
    ...
    .onDelete(async (snapshot, context) => {
      try {
        ...
        functions.logger.info(`Elemento con ID ${context.params.userId} se archivó correctamente. ${currentDate}.`, { structuredData: true });

        return null;
      } catch (error) {
        console.error('Error elemento eliminado:', error);
        return null;
      }
    });